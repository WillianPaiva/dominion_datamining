...
public class Elo {
    ...
    public  HashMap<String,Integer> Calculate(ArrayList<String> winners, HashMap<String,Integer>players){
        double eloPool = 0;
        ...
        for(Map.Entry<String,Integer> entry: players.entrySet()){
            if(winners.contains(entry.getKey())){
                ...
                temp.put(entry.getKey(),(int)(entry.getValue()+((32*(0-(Math.pow(10,entry.getValue()/400)/eloPool)))*factor)));
            }
