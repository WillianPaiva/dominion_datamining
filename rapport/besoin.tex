...
\begin{homeworkProblem}[Présentation du projet]
  ...
  Le jeu de cartes Dominion a été mis a disposition sur un serveur de jeu de 2010 à 2013. Les parties effectuées via ce serveur ont été enregistrées dans des logs et sont a disposition du public. Le but de ce projet proposé par Yvan Le Borgne, chercheur au Labri, est de traiter ces logs afin de répondre à plusieurs interrogation.
  \newline Un wiki relatif au jeu est disponible et propose une expertise sur ce jeu, notamment au niveau des stratégies proposées. Les données recueillies par notre programme devront être comparées au préconisations de ce wiki afin de valider ou non les informations fournies. Pour cela notre programme devra tout d'abord extraire les données présentes dans les logs car ceux-ci sont trop volumineux et trop compliqués à utiliser directement (structure changeante). Dans cette optique, le projet a pour but de créer une base de données recensant les parties puis un travail d'analyse sera effctuée à partir de celle-ci. Enfin une manière simple de représenter les données extraites et analysées sera proposée à l'utilisateur.
