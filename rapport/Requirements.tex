...
%In order to allow the user to ask for specific statistics, the program can be run in order to generate a result of a certain query and then a python script will be applied to this result. This would allow more flexibility in the type of statistics displayable by the program.
%\begin{itemize}
%\item the program will be launched by using the following command: \textit{dominionmining data\_to\_query user\_script.py}
% \item the python script will be applied to the generated query and return the statistics asked by the user.
%\item The generated file containing the query results will have the following format:
%\begin{itemize}
%\item file name?
%\item for each value or list of values returned, the first line will be the name of the value (winner, game, player names)
%\item then the following lines will contain the values, in case of a game description, each line will contain an action.(TODO: provide game description format)
%\end{itemize}
%\end{itemize}
...
%TODO: The program will have to be able to execute python scripts created by the user.
%TODO: The data generated will have to be analyzable by the user in order to allow as much flexiblity as possible in the treatment of the data.
